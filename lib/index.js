...
internals.registerPack = function (server, routePacks, filename) {
	...
	server.pack.register({
		name: routePackPlugin.name || filename,
		version: routePackPlugin.version || '1.0.0',
		register: function(plugin, options, next) {
			if (exports.viewOptions) {
				plugin.path(Path.resolve(routePacks + '/..'));

				plugin.ext('onPostHandler', function(request, reply){
					if(request.response.variety !== 'view') return reply();
 				...
 				var viewOptions = exports.viewOptions(plugin);
 				viewOptions.engines.html.module = internals.wrapHelpers(viewOptions.engines.html.module);

				plugin.views(viewOptions);
			}

			routePackPlugin.register(plugin, options, next);
		}
	}, function(err) {
		if(err) {
